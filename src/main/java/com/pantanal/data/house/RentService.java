...
public class RentService {
    ...
    private void aggrQuote(RentQuoteEntity quote , RentHouse houseInfo){
        ...
        if(quote.getHigh() < houseInfo.getPrice()) quote.setHigh(houseInfo.getPrice());
        if(quote.getLow() > houseInfo.getPrice()) quote.setLow(houseInfo.getPrice());

        double houseAvg = houseInfo.getPrice() / houseInfo.getSquare();
        if(quote.getHighSquareMeter() < houseAvg) quote.setHighSquareMeter(houseAvg);
        if(quote.getLowSquareMeter() > houseAvg) quote.setLowSquareMeter(houseAvg);
    ...
    public List<RentHouse> extractRentHouse(Object[] datas , String cityName){
        ...
        for(Object data : datas){
            ...
            if(!price.isEmpty())h.setPrice(new Double(price.get(0).toString()));
            ...
            if(!cl.isEmpty()) h.setCommunityName(cl.get(0).toString());

            List ll = m.get("landmark");
            if(!ll.isEmpty()) h.setLandmark(ll.get(0).toString());

            List dl = m.get("distirct");
            if(!dl.isEmpty()) h.setDistrict(dl.get(0).toString());
